/* =====================================================================
   NODRA ACADEMY — LESSON 02: WALLETS & SELF CUSTODY
   ...
   - Reading progress, scrollspy, copy-to-clipboard and the quiz engine
     mirror blockchain.js (Module 01) exactly, duplicated here because
     Module 01's rules forbid editing blockchain.js directly. See
     "Future Global Improvements" for the proposal to extract this into
     one shared lesson.js.
   ===================================================================== */
...
(function () {
  ...
  /* ===================================================================
     Copy-to-clipboard — shared by wallet diagram values
     =================================================================== */
  ...
  /* ===================================================================
     Seed phrase demo — blurred by default; a button reveals or hides
     it again, reinforcing the "always keep it hidden" habit.
     =================================================================== */
  function initSeedDemo() {
    var grid = document.getElementById('seedGrid');
    var toggleBtn = document.getElementById('seedToggleBtn');
    if (!grid || !toggleBtn) return;

    var showLabelEl = document.getElementById('i18nSeedShowLabel');
    var hideLabelEl = document.getElementById('i18nSeedHideLabel');

    toggleBtn.addEventListener('click', function () {
      var isHidden = grid.classList.toggle('is-hidden');
      var label = isHidden
        ? (showLabelEl ? showLabelEl.textContent.trim() : 'Show phrase')
        : (hideLabelEl ? hideLabelEl.textContent.trim() : 'Hide phrase');
    ...
    function grantModuleBadge() {
      ...
      window.nodraSupabase.auth.getSession().then(function (result) {
        ...
        fetch(window.nodraSupabaseUrl + '/functions/v1/academy-grant-module-badge', {
          ...
          body: JSON.stringify({ module_slug: '02-wallets' }),
        })
