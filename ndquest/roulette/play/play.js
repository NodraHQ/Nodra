...
function reactToRoomState(room) {
    // Sala encerrada - a Roleta nunca tinha essa reação (só criava a
    // sala com status 'open' e nunca reagia a nenhuma outra coisa).
    // Reportado ao vivo: veio junto do pedido de limitar conexão
...
function subscribeToRoomUpdates(roomId, nickname) {
    roomRealtimeChannel = window.ndquestSupabase
        ...
        .subscribe((status) => {
            // Presence - reportado ao vivo: "tem como saber se a
            // pessoa tá acompanhando ou se fechou a tela?". Diferente
            // de postgres_changes (que só reage a mudança na linha do
            // banco), Presence é o próprio Realtime avisando quando
            // uma conexão cai de verdade - fecha a aba, perde
            // internet, o navegador mata a aba em segundo plano -
            // sem precisar de nenhum evento manual (beforeunload não
            // é confiável pra chamada assíncrona, o navegador
            // costuma matar a página antes dela terminar). O host
            // escuta esse mesmo canal do lado dele pra saber quem
            // ainda está de verdade conectado.
            if (status === 'SUBSCRIBED' && nickname) {
                roomRealtimeChannel.track({ nickname, online_at: new Date().toISOString() });
            }
        });
}
...
joinRoomBtn.addEventListener('click', async () => {
    ...
    // Roleta nunca conferia status fechado na hora de entrar (a sala
    // simplesmente não tinha esse conceito até agora). Some com o
    // check de inatividade, virou o mesmo padrão dos outros 3 jogos.
    ...
    if (isRoomStale(data.updated_at)) {
        await window.ndquestSupabase.from('roulette_rooms').update({ status: 'closed' }).eq('id', data.id);
        joinError.textContent = t('errors.roomClosed');
        return;
    }
    ...
    if (!alreadyJoined) {
        if (joinUserId && !(await isAnonymousSession())) {
            window.ndquestSupabase
                .from('match_history')
                .insert({
                    user_id: joinUserId,
    ...
    subscribeToRoomUpdates(data.id, nickname);
});
